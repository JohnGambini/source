    ...
    QWidget(parent)
{
    ...
    vLayout->setSpacing(0);
    vLayout->setMargin(0);
    m_panSlider = new QSlider(Qt::Vertical);
    m_panSlider->setMinimum(1);
    m_panSlider->setMaximum(128);
    m_panSlider->setSliderPosition(64);
    m_panSlider->setStyle(QStyleFactory::create("cleanLooks"));
    m_panSlider->setTickPosition(QSlider::TicksAbove);
    vLayout->addWidget(m_panSlider,2,Qt::AlignHCenter);

    QLabel * panLabel = new QLabel;
    panLabel->setText("Pan");
    panLabel->setAlignment(Qt::AlignHCenter);
    panLabel->setStyleSheet("background-color:transparent");
    vLayout->addWidget(panLabel);
    vLayout->setAlignment(Qt::AlignHCenter);

    layout->addLayout(vLayout,0,3,2,1);
...
void PatchChnlWidget::setRoot( gnaTreeItem * item )
{
    ...
    m_panSlider->setValue(pan.toInt());
    blockSignals(false);
}
...
void PatchChnlWidget::panChanged(int value)
{
    if( m_root == NULL )
        return;
